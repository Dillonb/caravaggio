...
GLuint Cylinder::_base_offset[3] = {0,0,0};
GLuint Cylinder::_element_buffer_object = 0;
GLuint Cylinder::_vertex_array_object = 0;
GLuint Cylinder::_vertex_buffer = 0;
GLint Cylinder::_SL_AmbientRho = 0;
GLint Cylinder::_SL_DiffuseRho = 0;
GLint Cylinder::_SL_SpecularRho = 0;
GLint Cylinder::_SL_Shininess = 0;
GLenum Cylinder::_mode[3] = {GL_TRIANGLE_FAN, GL_TRIANGLE_STRIP, GL_TRIANGLE_FAN};
GLsizei Cylinder::_count[3] = {_nWedges + 2, (_nSlices - 1)*(2*(_nWedges + 1) + 1) - 1, _nWedges + 2};
GLsizei Cylinder::_nElements{_nSlices*(2*_nWedges + 3)};


void Cylinder::initialize_class() {
    _base_offset[0] = 0;
    ...
    for (int slice = 0; slice < _nSlices; slice++) {
        ...
        for (int wedge = 0; wedge < _nWedges; wedge++) {
            ...
            position[index][0] = std::cos(phi);
            position[index][1] = std::sin(phi);
            position[index][2] = height;

            normal[index][0] = position[index][0] - position[0][0];
            normal[index][1] = position[index][1] - position[0][1];
            normal[index][2] = 0.0f;

            index++;
        }
    }
    // index shoud now equal nCylinderVertices - 1
    ...
    // constitute the waist of the cylinder.
    int lower_offset;
    int upper_offset= 1;
    ...
    // position 1. Note that for a unit cylinder, the position and normal vectors are equal.
...
void Cylinder::draw_elements() {
   ...
   glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _element_buffer_object);
   for (int i = 0; i < 3; i++) {
      glDrawElements(Cylinder::_mode[i], Cylinder::_count[i], GL_UNSIGNED_SHORT,
                     (const GLvoid *) (Cylinder::_base_offset[i]*sizeof(GLushort)));
   }
}

glm::mat4 Cylinder::model_transformation() {
   return _model_transformation;
}

glm::mat3 Cylinder::vector_transformation() {
   return _vector_transformation;
}

glm::vec3 Cylinder::ambient_rho() {
   return _ambient_rho;
}

void Cylinder::set_ambient_rho(glm::vec3 rho) {
   _ambient_rho = rho;
}

void Cylinder::set_ambient_rho_SL(GLint sl) {
   _SL_AmbientRho = sl;
}

glm::vec3 Cylinder::diffuse_rho() {
   return _diffuse_rho;
}

void Cylinder::set_diffuse_rho(glm::vec3 rho) {
   _diffuse_rho = rho;
}

void Cylinder::set_diffuse_rho_SL(GLint sl) {
   _SL_DiffuseRho = sl;
}

glm::vec3 Cylinder::specular_rho() {
   return _specular_rho;
}

void Cylinder::set_specular_rho(glm::vec3 rho) {
   _specular_rho = rho;
}

void Cylinder::set_specular_rho_SL(GLint sl) {
   _SL_SpecularRho = sl;
}

GLfloat Cylinder::shininess() {
   return _shininess;
}

void Cylinder::set_shininess(GLfloat s) {
   _shininess = s;
}

void Cylinder::set_shininess_SL(GLint sl) {
   _SL_Shininess = sl;
}
